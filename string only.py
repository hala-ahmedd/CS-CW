...
def is_prime(): #function to check if a number is prime or not
        global prime 
        prime=random.randint(200,10000) #generates the prime number randomly
...
def generating_keys (prime): #function to generate all the keys needed
    global x #global scope to use it outside the function's local scope
...
def decryption(c1,c2,prime,x): # function to decrypt a message using ElGamal 
    ...
    original_message=(c2* z_inverse) % prime # decrypted message = c2 * z_inverse mod p
...
def message_to_number(message): #function to convert the letters into their ASCI code
    global lst
    lst=[] #empty list
    if type(message)==str: #extra checkpoint for clarity
...
def main(message): #main function: the start of implementation of every function
    check=is_prime()
    while check==False: #making sure that the random number is a prime number
            check=is_prime() 
    ...
    for let_num in lst1: #let_num= letter_number
        b = random.randint(2, prime-2) #generates b randomly
        c1,c2 = encryption(let_num, prime, g, y,b)  # encrypt each character
        encryptionlst.append((c1, c2))  # append encrypted pair to the encryption list
    ...
    print(f"Decrypted message: {''.join(decryptionlst)}") #joins all the values in the list to make it one string
    print("Process is done.") #confirming to the user that the process is done
main(message)
